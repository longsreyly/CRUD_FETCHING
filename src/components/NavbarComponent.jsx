        ...
        <div className={`${isMenuOpen ? "block" : "hidden" } items-center justify-between w-full md:flex md:w-auto md:order-1`} id="navbar-search">
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-gray-100 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            {navLink.map(({ href, label }) => (
              <li key={href}>
                <Link href={href} className={`block py-2 pl-3 pr-4 rounded ${pathname === href ? "underline" : "" } hover:bg-gray-100 md:hover:text-pink-700 dark:text-white dark:hover:bg-gray-700`} >
                  {label}
                </Link>
              </li>
            ))}
